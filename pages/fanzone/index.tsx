...
const Fanzone = () => {
  ...
  const handleFetchFeedMoreData = async () => {
    setPageData((prev) => ({
      ...prev,
      posts: [],
    }));
    setPageError(false);
    setFeedMoreDataLoading(true);
    try {
      let response = await callAPI({
        endpoint: `/v1/fanzone/publicpost?page=${page + 1}`,
      });
  ...
  return (
    <div className="w-full flex flex-col h-full rounded-lg border-2 border-white border-opacity-10 bg-white bg-opacity-5 space-y-4 p-5">
      ...
      <div className="relative w-full flex justify-between items-center">
        ...
        <div className="px-3 py-2 border border-white border-opacity-10 bg-white bg-opacity-5 text-[10px] font-inter font-semibold cursor-pointer rounded-lg" onClick={() => router.push(`/tribes`)}>
          See More
        </div>
      </div>
