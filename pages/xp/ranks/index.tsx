...
import ToggleSwitch from '../../../core/Atoms/Inputs/ToggleSwitch';
...
const Index = () => {
    ...
    if (ranks) {
      {
        setSelectedTab("TRENDING"),
          setLeaderList(ranks?.trendingLeaderboard);
      }
    }
  }, [ranks]);
  ...
  useEffect(() => {
    if (selectedTab == "TRENDING") {
      ...
      setLeaderList(ranks?.topLeaderboard);
    }
  }, [selectedTab]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const response = await callAPI({endpoint : "/v1/xp/ranks"});
    ...
    <XpLayout>
      ...
      <div className="flex justify-start items-center gap-20">
        ...
        <div className="flex-1 w-full relative">
          <ListTabel list={leaderList} loading={loading} />
        </div>
